...
@app.route("/")
def index():
    next_class_obj = calculations.get_closest_class(calculations.get_day_timetable('monday'))
    next_homework_obj = homework.get_next_task_due()
    print(type(next_homework_obj))
    return render_template('index.html', next_class = next_class_obj, readable_time = calculations.return_readable_time(next_class_obj['start']), next_homework = next_homework_obj, homework_time = calculations.return_readable_time(next_homework_obj['deliver_time']))
